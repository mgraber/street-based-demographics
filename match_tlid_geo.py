...
def import_data(county_code = '08031', spatial = True, sample=True):
    ...
    county_address_df = pd.read_csv("addresses/" + county_code + "_addresses.csv")
...
def simplify_road(edges, county_code='08031', tol=10):
    ...
    if not os.path.exists("tiger_csv/simplified_edges/"):
        os.mkdir("tiger_csv/simplified_edges/")
    edges.to_file(driver = 'ESRI Shapefile', filename = "tiger_csv/simplified_edges/" + county_code + "_simp_" + str(tol).replace(".",""))
    return edges
...
def run_distance_calc(county_code = '08031', spatial = True, simplify = False, tol = 0, mids=False, sample = True):
    """
    Finds the TLID closest to the point, given that the TLID is one of the options
    found using the tiger_xwalk.py crosswalk
    ...
    maf_has_tlid.loc[:,'TLID_match_geo'] = maf_has_tlid.apply(lambda row: row['TLIDs'][0], axis=1)
    ...
    if spatial==True:
        ...
        match_t0 = time.time()
        maf_needs_tlid.loc[:,'TLID_match_geo'] = maf_needs_tlid.apply(lambda row: min_dist_geo(row, edges), axis=1)
        match_t1 = time.time()
    ...
    if sample:
        outfile_name = "address_tlid_xwalk/" + county_code + "samp_geo_match.csv"
    else:
        outfile_name = "address_tlid_xwalk/" + county_code + "_geo_match.csv"
